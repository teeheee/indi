...
bool LX200_OnStep::updateProperties()
{
    ...
    if (isConnected())
    {
        ...
        // Focuser
        if(OnStepMountVersion == OSV_OnStepX || OnStepMountVersion == OSV_UNKNOWN)
        {
            LOG_INFO("Version is OnStepX or Unknown, create focuser based on commands");
            for (int i = 0; i < 9; i++)
            {
                ...
                if (fail_or_error > 0 && read_buffer[0] == '1')  // Do we have a Focuser X
                {
                    LOGF_INFO("Focuser %i Found", i);
                    std::unique_ptr<LX200_OnStep_Focuser> focuser(new LX200_OnStep_Focuser(PortFD, i, OnStepMountVersion));
                    focuser_list.push_front(std::move(focuser));
                }
                else
                {
                    LOGF_INFO("Focuser search end %d %c", fail_or_error, read_buffer[0]);
                    break;
                }
            }
        }
        ...
        {
            LOG_INFO("Version is OnStep, create first focuser");
            ...
            {
                std::unique_ptr<LX200_OnStep_Focuser> focuser(new LX200_OnStep_Focuser(PortFD, 0, OnStepMountVersion));
                focuser_list.push_front(std::move(focuser));
            }
            error_or_fail = getCommandSingleCharResponse(PortFD, response, ":fA#"); //0 = failure, 1 = success, no # on reply
            if (error_or_fail > 0 && response[0] == '1')
            {
                std::unique_ptr<LX200_OnStep_Focuser> focuser(new LX200_OnStep_Focuser(PortFD, 1, OnStepMountVersion));
                focuser_list.push_front(std::move(focuser));
            }
        }
